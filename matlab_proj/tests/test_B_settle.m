...
% шага, up = phi_next(c-1,:). Эталоны в ref_dyn/ перевыгружены под неё,
% поэтому сверка идёт напрямую по CSV на обоих уровнях.
%
...
for tag = {'3ph', '2ph'}
    ...
    [cap, phi_next] = tric_settle_dyn(s.phi_s0, ps.w, Ts1_mat, ...
                                      zeros(n, J), v.tau_s, dt);
    ...
    [cap2, phi2] = tric_settle_dyn(s.phi_s0, ps.w, T, P, vs.tau_s, dt);
    ...
    [~, phiQ] = tric_settle_dyn(s.phi_s0, ps.w, Ts1_mat, ...
                                zeros(n, J), v.tau_s, 1e9);
    up   = s.phi_s0 * ps.w.';
    star = up .* (1 - Ts1_mat(1,:));
